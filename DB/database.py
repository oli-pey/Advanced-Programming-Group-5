...
class PredictionEntry(Base):
    ...
    downsized_image: Mapped[bytes | None] = mapped_column(
        LargeBinary,
        nullable=True,
    )
...
class SandboxClass(Base):
    ...
    dataset_id: Mapped[int] = mapped_column(
        ForeignKey('sandbox_datasets.id'),
        nullable=False,
    )
    name: Mapped[str] = mapped_column(
        String(100),
...
class SandboxTrainedModel(Base):
    ...
    training_job_id: Mapped[int] = mapped_column(
        ForeignKey('sandbox_training_jobs.id'),
        nullable=False,
    )
    owner_user_id: Mapped[int] = mapped_column(
        ForeignKey('users.id'),
    ...
    class_index_json: Mapped[str] = mapped_column(
        Text,
        nullable=False,
    )
    ...
    is_promoted_to_main_ui: Mapped[bool] = mapped_column(
        Boolean,
        default=False,
        nullable=False,
    )
